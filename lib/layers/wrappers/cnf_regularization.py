...
def l1_regularzation_fn(x, t, logp, dx, dlogp, unused_context):
    del x, t, logp, dlogp
    return torch.mean(torch.abs(dx))


def l2_regularzation_fn(x, t, logp, dx, dlogp, unused_context):
    del x, t, logp, dlogp
    return _batch_root_mean_squared(dx)


def directional_l2_regularization_fn(x, t, logp, dx, dlogp, unused_context):
    del t, logp, dlogp
    directional_dx = torch.autograd.grad(dx, x, dx, create_graph=True)[0]
    return _batch_root_mean_squared(directional_dx)


def jacobian_frobenius_regularization_fn_ori(x, t, logp, dx, dlogp, context):
    del t, logp, dlogp
...
def jacobian_diag_frobenius_regularization_fn(x, t, logp, dx, dlogp, context):
    del t, logp, dlogp
    if hasattr(context, "jac"):
        jac = context.jac
    else:
...
def jacobian_offdiag_frobenius_regularization_fn(x, t, logp, dx, dlogp, context):
    del t, logp, dlogp
    if hasattr(context, "jac"):
        jac = context.jac
    else:
...
def quadratic_cost(x, t, logp, dx, dlogp, unused_context):
    del x, t, logp, dlogp, unused_context
